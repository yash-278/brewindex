...
async function runSync() {
  ...
  console.log('[sync] background sync complete', { synced: rows.length, icons_uploaded: uploadCount, icons_fallback: fallbackCount, github_enriched: githubEnriched, github_failed: githubFailed });
}
...
export async function syncHandler(c: Context) {
  ...
  // Respond 202 immediately so the cron trigger (Node fetch / undici) does not hit
  // its 30s headersTimeout while waiting for the multi-minute sync to complete.
  // All sync work runs as a detached background Promise.
  setImmediate(() => {
    runSync().catch((err) => console.error('[sync] background error', err));
  });
